...
public class Funcions {
    ...
    public void cercaUbicacio() {
        ...
        try {
            ...
            localizacion[0] = input.nextDouble();
            input.nextLine();

            System.out.println("Longitud:");
            localizacion[1] = input.nextDouble();
            input.nextLine();

            System.out.println("Radi màxim:");
            radi = input.nextInt();
            input.nextLine();
            ArrayListt postArray = new ArrayListt();

            importer.rTree.busquedaPorLocalizacion(localizacion, radi, importer.rTree.getrTreeRoot(), postArray);

            for (int i = 0; i < postArray.getSize(); i++){
                System.out.println("post con ID " + ((Post)postArray.getElement(i)).getId() + " encontrado!");
            }
